...
(function ($, undefined) {
	...
	eventNamePropertyChange = "propertyChange";
    ...
    function bind(obj, wrapped, handler) {
        wrapped.bind(obj.nodeType ? "change" : eventNamePropertyChange, handler);
    }
    function unbind(obj, wrapped, handler) {
        wrapped.unbind(obj.nodeType ? "change" : eventNamePropertyChange, handler);
    }
    ...
    $.extend({
        ...
        setField: function (target, field, value) {
            if (target.nodeType) {
                ...
                throw "Use $.observable(...).setProperty";
            }
        }
    });
    ...
    function getMapping(ev, changed, newvalue, map) {
		...
		isSetData = ev.type === eventNamePropertyChange,
		mappedName,
		convert,
		name;
        if (isSetData) {
        ...
        link: function (target, mapping) {
			...
			handler = function (ev, changed, newvalue) {
			    ...
			    if (m) {
			        ...
			        if (value !== undefined) {
			            $.observable(target).setProperty(name, value);
			        }
			    }
			},
			handlerRev = function (ev, data) {
			    // a change or changeData event occurred on the target,
			    // update the corresponding source elements
			    var newvalue = data.value,
				m = getMapping(ev, data.path, newvalue, mapRev);
			    if (m) {
			        var name = m.name,
						value = m.value,
						convert = m.convert;
